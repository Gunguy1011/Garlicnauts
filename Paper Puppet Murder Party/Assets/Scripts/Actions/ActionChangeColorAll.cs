...
public class ActionChangeColorAll : Action
{
    List<Color> StartFade_;
    ...
    public ActionChangeColorAll(MeshRenderer[] _objects, List<Color> start, List<Color> end, float duration = 0.0f, float time = 0.0f, EaseType type = 0, bool block_ = false, int blocknum_ = 0)
    {
        objects = _objects;   
        StartFade_ = start;
        EndFade_ = end;
    ...
    override public bool Update()
    {
        ...
        {
            Color newalpha = StartFade_[i] + (EndFade_[i] - StartFade_[i]) * PercentageDone_;
            objects[i].material.color = newalpha;
        }
