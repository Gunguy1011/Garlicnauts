...
public class ActionFadeCanvasElement : Action
{
    ...
    public ActionFadeCanvasElement(GameObject objectM, float start, float end, float duration = 0.0f, float time = 0.0f, EaseType type = 0, bool block_ = false, int blocknum_ = 0)
    {
        Actoor = objectM;
        StartFade_ = start;
        EndFade_ = end;
    ...
    override public bool Update()
    {
        ...
        Actoor.GetComponent<CanvasRenderer>().SetColor(new Color(Actoor.GetComponent<CanvasRenderer>().GetColor().r, Actoor.GetComponent<CanvasRenderer>().GetColor().g, Actoor.GetComponent<CanvasRenderer>().GetColor().b, newalpha));
